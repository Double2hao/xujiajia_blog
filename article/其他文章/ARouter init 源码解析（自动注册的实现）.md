...
1. 通过使用注解解释器生成辅助类。1. 通过使用动态编译（Transform+Plugin）的方式，调用注解解释器生成的辅助类，来讲注解中的内容注册。
>  
...
主要逻辑整理一下：
1. 先通过ScanUtil来扫描每一个类1. 再通过RegisterCodeGenerator来插入代码
...
1. 在编译阶段。注解解释器会自动生成辅助类，这些辅助类会继承IRouteGroup、IProvider、IRouteRoot这些interface。1. 在编译结束，到虚拟机执行之前。会根据这些interface，将自动生成的这些辅助类过滤出来，然后在loadRouterMap.loadRouterMap() 中调用loadRouterMap.register()来将这些辅助类中的内容自动注册。